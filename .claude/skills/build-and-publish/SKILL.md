...
- **Develop / NINA 3.3 line.** The workflow tag filter is scoped to `3.1.*` and `3.2.*`. A `release/v3.3.*` tag won't trigger anything. Cutting 3.3 releases needs a separate workflow path.
- **Working tree dirty, or HEAD not pushed.** Tagging an unpushed commit means the tag refers to a SHA only on your machine; pushing the tag will succeed but the workflow checks out the tag and builds from it — fine — but the release will point at a commit no one else has on a branch. Push the branch first.
...
```bash
# Branch + clean tree
git rev-parse --abbrev-ref HEAD          # must be release/3.2
git status --porcelain                   # must be empty

# HEAD must be pushed
git fetch origin
git rev-parse HEAD                       # local HEAD
git rev-parse origin/release/3.2         # must equal local HEAD
```
...
Assert the version starts with `3.2.` (the workflow's tag regex). If it's `3.1.*` we're on the wrong branch line for this skill; if it's `3.3.*` the workflow won't trigger. Either way, stop.

Compose the tag:

```bash
VERSION=3.2.0.2                          # from grep above
...
# Manifest PR (look at the publish-manifest job log, or just point at the PR list)
...
| Symptom | Cause | Fix |
|---|---|---|
| Workflow never starts after push | Tag doesn't match regex (e.g. `release/v3.2.0` — only 3 segments) | Delete tag (`git tag -d`, `git push origin :refs/tags/<tag>`), use 4-segment version |
| Workflow never starts after push | Tag is on `3.3.*` line | Wrong branch / wrong skill — 3.3 has no release workflow yet |
...
| `publish-manifest` skipped | `check-repo-exists` returned `exists=false` — ghilios/nina.plugin.manifests fork is missing | Fork `isbeorn/nina.plugin.manifests` to your account, re-run the workflow from the Actions tab (or re-push the tag after deleting it) |
| `publish-manifest` fails at `gh pr create` | PAT secret missing or lacks `repo` scope on the manifests fork | Renew the `PAT` repo secret with `repo` scope and re-run the failed job |
